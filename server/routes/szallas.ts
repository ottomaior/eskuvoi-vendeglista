...
router.get('/', async (_req, res) => {
  const result = await pool.query(
    "SELECT * FROM guests WHERE source = 'szallas' ORDER BY created_at"
  );
  res.json(result.rows);
});

// ── POST /api/szallas (bulk replace) ─────────────────────────────────────────
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    await client.query("DELETE FROM guests WHERE source = 'szallas'");
...
// ── DELETE /api/szallas (clear all szallas guests) ────────────────────────────

router.delete('/', async (_req, res) => {
  await pool.query("DELETE FROM guests WHERE source = 'szallas'");
  await broadcast();
  res.json({ ok: true });
});
